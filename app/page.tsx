...
// Inilipat dito ang useSearchParams para ma-wrap ng Suspense sa main Page export
  ...
  return (
    <div className="min-h-screen bg-[#F0F4F9] text-[#1A1C1E] p-4 md:p-8 tracking-normal">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Hidden reader for image scanning */}
            ...
            <div className="w-full max-w-sm text-center">
              ...
              <div className="space-y-4">
                ...
                <button 
                  ...
                  className="w-full bg-[#005FB7] text-white py-4 rounded-full font-semibold text-sm hover:shadow-md transition-all disabled:opacity-50 flex items-center justify-center gap-3"
                >
                ...
                <div className="lg:col-span-7 p-8 md:p-12 space-y-10">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {[
                      ...
                      <div key={idx} className="bg-[#F7F9FF] p-6 rounded-[28px] border border-[#D3E3FD]/30">
                  ...
                  <div className="space-y-6 pt-4">
                    <div className="space-y-3">
                      <h4 className="text-sm font-semibold text-[#44474E] ml-2">Maintenance history</h4>
                      <div className="p-7 bg-[#FDFBFF] border border-[#E0E2EC] rounded-[32px] text-sm text-[#44474E]">
                        {selectedItem.maintenanceRecords || "No logged maintenance history available."}
                      </div>
                    </div>
                  </div>
                </div>

                <div className="lg:col-span-5 bg-[#F7F9FF] p-8 md:p-12 border-l border-[#E0E2EC]/50">
                  <p className="text-sm font-semibold text-[#74777F] mb-8 text-center">Digital documentation</p>
                  {(() => {
                    const fileIdMatch = selectedItem?.gdriveLink?.match(/\/d\/([a-zA-Z0-9_-]{25,})/);
                    const fileId = fileIdMatch ? fileIdMatch[1] : null;
                    if (fileId) {
                      return (
                        <div className="relative w-full rounded-[40px] border border-[#D3E3FD] bg-white shadow-2xl overflow-hidden h-[600px]">
                    ...
                    return (
                      <div className="h-[600px] w-full flex items-center justify-center border-2 border-dashed border-[#D3E3FD] rounded-[40px] bg-white/50 p-10 text-center text-[#74777F]">
                        <p className="font-medium opacity-50 text-sm">No documentation attached.</p>
                      </div>
                    );
                  })()}
                </div>
      ...
      {isInvalidModalOpen && (
        <div className="fixed inset-0 bg-[#001C38]/40 backdrop-blur-md flex items-center justify-center p-4 z-[200]">
          <div className="bg-[#FDFBFF] rounded-[32px] p-8 w-full max-sm text-center shadow-2xl border border-[#D3E3FD]">
            <h2 className="text-xl font-medium mb-3">Invalid record</h2>
            <p className="text-[#44474E] text-sm mb-10">The code was not found. Please try again.</p>
            <button onClick={() => setIsInvalidModalOpen(false)} className="w-full bg-[#005FB7] text-white py-4 rounded-full font-semibold cursor-pointer">Close and retry</button>
          </div>
        </div>
      )}
    </div>
